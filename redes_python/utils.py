...
def plvar(x, *varargin):
    ...
    if f_dattype == 'REAL':
        ...
        for B in range(0, Bt):
            ...
            if not quiet:
                print '[' + str(B + 1) + ']\tntail = ', round(mean(bofA), 3), ' (', round(std(bofA),
                                                                                          3), ')', '\txmin = ', \
                    round(mean(bofB), 3), ' (', round(std(bofB), 3), ')', '\talpha = ', round(mean(bofC),
                                                                                              3), ' (', round(std(bofC),
                                                                                                              3), ')'
        ...
        for B in range(0, Bt):
            ...
            for xm in range(0, len(ymins)):
                ...
                # compute KS statistic
                fit = reduce(lambda X, Y: X + [Y + X[-1]], \
                             (map(lambda X: pow(X, -vec[I]) / (float(zvec[I]) - sum(
                                 map(lambda X: pow(X, -vec[I]), map(float, range(1, xmin))))), range(xmin, ymax + 1))),
                             [0])[1:]
                cdi = []
...
def plpva(x, xmin, *varargin):
    ...
    if f_dattype == 'REAL':
        ...
        L = []
        for k in range(0, len(vec)):
            L.append(-vec[k] * float(sum(map(log, z))) - float(nz) * log(
                float(zvec[k]) - sum(map(lambda X: pow(float(X), -vec[k]), range(1, xmin)))))
        I = L.index(max(L))
        alpha = vec[I]
        fit = reduce(lambda X, Y: X + [Y + X[-1]], \
                     (map(lambda X: pow(X, -alpha) / (
                                 float(zvec[I]) - sum(map(lambda X: pow(X, -alpha), map(float, range(1, xmin))))),
        ...
        for B in range(0, Bt):
            ...
            for qm in range(0, len(qmins)):
                ...
                if nq > 1:
                    ...
                    for k in range(1, len(vec)):
                        L.append(-vec[k] * float(slogzq) - float(nq) * log(
                            float(zvec[k]) - sum(map(lambda X: pow(float(X), -vec[k]), qminvec))))

                    I = L.index(max(L))
                    fit = reduce(lambda X, Y: X + [Y + X[-1]], \
                                 (map(lambda X: pow(X, -vec[I]) / (float(zvec[I]) - sum(
                                     map(lambda X: pow(X, -vec[I]), map(float, range(1, qmin))))),
                                      range(qmin, qmax + 1))), [0])[1:]
                    cdi = []
